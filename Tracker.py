...
keyboard = Controller()
cap = cv2.VideoCapture(1)  # Or 1 if any external webcam or mobile cam is used
#### List of trackers ####
# tracker = cv2.TrackerMOSSE_create()
# tracker = cv2.TrackerCSRT_create()  # (g)
...
bbox = cv2.selectROI("Tracking", img, None) # getting the coordinates of the selscted box
...
radius_Bound = 28 # radius of circle used for tracking
...
def get_direction(tracker_dot_x, tracker_dot_y):
    ...
    if count < 1:
        ...
        if tracker_dot_x > (center_x + radius_Bound):
            keyboard.press('d')
            keyboard.release('d')
...
while True:
    ...
    cv2.circle(img, (tracker_dot_x, tracker_dot_y), 6, (255, 0, 0), 2) # The tracker circle
    cv2.circle(img, (center_x, center_y), radius_Bound, (255, 255, 0), 2) # The bounding circle
